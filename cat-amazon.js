...
const cat_amazon = async (req, res) => {
    ...
    if (!req.query['keyword']) {
        return res.status(400).json({ error: 'Le paramètre mot-clé est requis.' });
    }
    if (!req.query['pages'] || isNaN(parseInt(req.query['pages']))) {
    ...
    try {
        ...
        await browser.close();  // Assurez-vous que le navigateur est fermé en cas d'erreur
